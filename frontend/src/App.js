...
import { MapPin, Users, Clock, Car, Bike, Navigation, Bus, Coffee, Utensils, TreePine, Activity, Plus, X, AlertCircle, CheckCircle, Map, Heart, Waves, Wine, SquareParking, Baby, Dog, Sun } from 'lucide-react';
...
function App() {
  ...
  // Calculate venue quality score based on rating and price
  const calculateVenueQuality = (venue) => {
    if (!venue.rating) return 0.5;
    
    // Convert 3-5 star rating to 0-1 score
    const ratingScore = Math.max(0, Math.min(1, (venue.rating - 3.0) / 2.0));
    
    // Price level bonus (mid-range gets slight boost)
    let priceBonus = 0;
    if (venue.priceLevel === 2) priceBonus = 0.1; // Mid-range sweet spot
    if (venue.priceLevel === 3) priceBonus = 0.05; // High-end slight bonus
    
    return Math.min(1, ratingScore + priceBonus);
  };

  // Calculate travel fairness with tolerance (no one travels significantly more)
  const calculateTravelFairness = (durations) => {
    ...
    // Calculate tolerance (20% of average time, minimum 10 minutes)
    const tolerance = Math.max(10, avgTime * 0.20);
    
    // Calculate unfairness (how much the longest journey exceeds average)
    const unfairness = Math.max(0, maxTime - avgTime);
    
    // Score based on how much unfairness exceeds tolerance
    if (unfairness <= tolerance) {
      return 1.0; // Perfect fairness within tolerance
    } else {
      // Linear penalty for exceeding tolerance
      const excessUnfairness = unfairness - tolerance;
      const penalty = Math.min(1, excessUnfairness / tolerance);
      ...
      if (transitStations && transitStations.length > 0) {
        ...
        if (optimalStation) {
          ...
          for (const station of prioritizedStations) {
            try {
              const stationWeight = 1 - (Math.abs(station.time - targetTime) / 1200);
    ...
    try {
      ...
      for (let venueIndex = 0; venueIndex < venuesWithDetails.length; venueIndex++) {
        ...
        if (travelTimes.length === friendCoords.length) {
          ...
          const avgTime = durations.reduce((sum, time) => sum + time, 0) / durations.length;
          const minTime = Math.min(...durations);
          const fairnessScore = maxTime - minTime;
          
          // OLD SCORING SYSTEM (working version)
          let score = (
          ...
          const venueData = {
            ...
            minTravelTime: Math.round(minTime),
