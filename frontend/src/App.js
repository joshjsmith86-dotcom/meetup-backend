...
function App() {
    ...
    try {
      ...
      if (data.status === 'OK' && data.routes.length > 0) {
        ...
        for (const leg of legs) {
          for (const step of leg.steps) {
            if (step.travel_mode === 'TRANSIT' && step.transit_details) {
              ...
              const estimatedStops = Math.floor(stepDuration / 120);
        ...
        for (const station of realCandidates) {
          ...
          const fairnessThreshold = totalDuration * 0.15;
        ...
        if (bestRealStation) {
          return {
            ...
            selectedOptimalStation: bestRealStation,
        ...
        for (const station of candidateStations) {
          ...
          try {
            const venueCount = await testVenueCountNearStation(station.location.lat, station.location.lng);
            const venueScore = Math.min(1, venueCount / 8);
            score += venueScore * 40;
          } catch (error) {
    ...
    if (data.status === 'OK') {
      ...
      for (const row of data.rows) {
        ...
        if (element.status === 'OK') {
          totalTime += element.duration.value / 60;
          validRoutes++;
        }
      }
  ...
  const isVenueOpenAt = (openingHours, dateTime, timezone = null) => {
    ...
    for (const period of todayPeriods) {
      ...
      if (period.close && typeof period.close.time === 'string') {
        closeTime = parseInt(period.close.time.padStart(4, '0')) || 0;
      }
      ...
      if (transitStations && transitStations.length > 0) {
        ...
        if (optimalStation) {
            ...
            if (data.status === 'OK') {
              ...
              if (venues.length < 8) {
                const nearbyStations = transitStations
                  .filter(station => station.name !== optimalStation.name)
                  .sort((a, b) => Math.abs(a.time - optimalStation.time) - Math.abs(b.time - optimalStation.time))
                  .slice(0, 2);
          ...
          const prioritizedStations = transitStations
            .filter(station => Math.abs(station.time - targetTime) <= 1200)
  ...
  // Listen for auto-search event
  useEffect(() => {
    const handleAutoSearch = () => {
      if (friends.every(f => f.location.trim() !== '')) {
        findOptimalMeetups();
      }
    };

    window.addEventListener('autoSearch', handleAutoSearch);
    return () => window.removeEventListener('autoSearch', handleAutoSearch);
  }, [friends, transport, activities, additionalFilters, customSearch, meetupDateTime]);
